...
function initialize() {
  ...
  if (storedForms) forms = JSON.parse(storedForms);
  if (storedResponses) responses = JSON.parse(storedResponses);
...
function showCreateForm() {
  ...
  (document.getElementById('formTitle')! as HTMLInputElement).value = '';
 ...
 */
function resetFormCreator(): void {
  (document.getElementById('formTitle')! as HTMLInputElement).value = '';
  document.getElementById('formFields')!.innerHTML = '';
}
 ...
 */
function addField(type: 'text' | 'radio' | 'checkbox', existingField?: FormField) {
  const field: FormField = existingField || {
    id: Date.now().toString(),
    type,
    label: '',
 ...
 */
function updateFieldLabel(fieldId: string, value: string) {
  const field = currentForm!.fields.find(f => f.id === fieldId);
 ...
 */
function saveForm() {
  currentForm!.title = (document.getElementById('formTitle') as HTMLInputElement).value;
 ...
 */
function previewForm(formId: string) {
...
function submitForm() {
  ...
  currentForm!.fields.forEach(field => {
    if (field.type === 'text') {
      const input = document.querySelector(`input[name="${field.id}"]`) as HTMLInputElement;
      formResponse.responses[field.id] = input.value;
    } else if (field.type === 'radio') {
      const selected = document.querySelector(`input[name="${field.id}"]:checked`) as HTMLInputElement;
      formResponse.responses[field.id] = selected ? selected.value : '';
    } else {
      const checked = Array.from(document.querySelectorAll(`input[name="${field.id}"]:checked`)) as HTMLInputElement[];
...
function editForm(formId: string) {
  currentForm = forms.find(f => f.id === formId)!;
  showView('formCreator');
  (document.getElementById('formTitle')! as HTMLInputElement).value = currentForm.title;
  document.getElementById('formFields')!.innerHTML = '';
